...
def htn_planning_recursive(state, goal_task, root_node, max_depth, capabilities_input, db, send_update_callback=None):
    ...
    success, updated_state = decompose(root_node, state, 0, max_depth, capabilities_input, goal_task,
...
@trace_function_calls
def htn_planning(initial_state, goal_task, capabilities_input, max_depth=5, send_update_callback=None):
    # Storage for successful task_node's so that they don't need to get regenerated for similar inputs
    db = None # db = VectorDB()
    root_node = TaskNode(goal_task)
    while replan_required(initial_state, goal_task, root_node):
        root_node = htn_planning_recursive(initial_state, goal_task, root_node, max_depth, capabilities_input,
                                           db, send_update_callback)
    return root_node


@trace_function_calls
def replan_required(state, goal_task, task_node):
    if gpt4_is_goal(state, goal_task):
        return False
...
@trace_function_calls
def decompose(task_node, state, depth, max_depth, capabilities_input, goal_state, db, send_update_callback=None,
              n_candidates=3):
    ...
    if remaining_decompositions == 0:
        ...
        if is_task_primitive(task, capabilities_input):
            ...
            translated_task = translate_task(task, capabilities_input)
            ...
            if can_execute(translated_task, capabilities_input, decompose_state):
                ...
                updated_state = execute_task(state, translated_task)
            ...
            for _ in range(n_candidates):
                subtasks_list = get_subtasks(task, decompose_state, remaining_decompositions, capabilities_input)
                score = evaluate_candidate(task, [subtask for subtask in subtasks_list], capabilities_input)
            ...
            for subtasks_list, score in candidates:
                if check_subtasks(task, [subtask for subtask in subtasks_list], capabilities_input):
            ...
            if success or best_candidate is not None:
                for subtask in subtasks_list:
                    ...
                    success, updated_state = decompose(subtask_node, decompose_state, depth + 1, max_depth,
                                                       capabilities_input,
